...
namespace iImagineC.WCF.Test
{
    ...
    public class SubscriberServiceTests
    {
        ...
        [TestMethod]
        public void SubscriberService_PublishToAll_MultithreadPubSub()
        {
            using (var host = new WcfHost())
            {
                List<Client> clients = new List<Client>();
                try
                {
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        host.instance.PublishToAll("Roman" + (int)index), i);
                    }


                    //Register some more clients
                    for (int j = 0; j < 99; j++)
                        ThreadPool.QueueUserWorkItem(index =>clients.Add(new Client(((int)index).ToString())), j);


                    host.instance.PublishToAllSerial("LastData", 1000000);
                    Thread.Sleep(60000);
                    ...
                    clients.ForEach(a =>
                    {
                        if (a.Data == "LastData") clientsWithData++;
                        else rogueClients++;
                    });
